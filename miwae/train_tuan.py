...
if __name__ == "__main__":
    ...
    # get dataloader
    train_dataset = Multi_view_data(args.data_path, train=True, 0)
    test_dataset = Multi_view_data(args.data_path, train=False, 0)
    ...
    for ep in range(1,args.epochs):
        # two training stage
        if ep < 30:
            step = [5, 0, 0] # 第一阶段，使用重建损失单独优化编码器
        else:
            step = [0, 0, 5] # 第二阶段，使用重建损失\分类损失\MCCA正则联合优化整个模型
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            for i in range(step[2]):
                ...
                for num in range(train_input_num-1):
                    #sign = (~(predict_list[num+1]==target)&(predict_list[num]!=target)).long()
